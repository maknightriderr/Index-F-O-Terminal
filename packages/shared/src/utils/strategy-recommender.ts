...
// and iron condors — premium selling the user doesn't trade. Rich IV steers
// toward a debit spread (the short leg offsets the expensive premium)
// instead of toward selling; mixed or neutral reads get nothing.
//
// Shared by the web page and the server's track-record job, so what the
// page recommends is exactly what gets graded.
// ============================================================

import { IV_RANK_HIGH_THRESHOLD } from '../constants/index.js';
import type { FnoScannerRow } from '../types/index.js';

export type ScannerStrategyCategory = 'DIRECTIONAL';
export type ScannerRiskProfile = 'DEFINED_RISK';

export interface ScannerStrategyRecommendation {
  strategy: string;
  category: ScannerStrategyCategory;
  riskProfile: ScannerRiskProfile;
  rationale: string;
}
...
export function recommendStrategy(row: FnoScannerRow): ScannerStrategyRecommendation | null {
